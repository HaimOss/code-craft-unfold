  ...
  const handleImportJSON = () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (payload.type !== 'trip') { toast({ title: 'הקובץ מכיל פעילות, לא טיול.', variant: 'destructive' }); return; }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto" dir="rtl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold font-display">הטיולים שלי</h1>
      ...
      {trips.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h2 className="text-2xl font-bold font-display mb-2">אין טיולים עדיין</h2>
          <p className="text-muted-foreground mb-6">התחל לתכנן את ההרפתקה הבאה שלך!</p>
          <button onClick={() => setIsAddTripModalOpen(true)} className="btn-primary flex items-center gap-2">
            <Plus className="h-4 w-4" /> צור את הטיול הראשון
          </button>
        </div>
      ) : (
