...
import { Plus, MapPin, Calendar, ArrowLeft, Clock, Plane, Hotel, UtensilsCrossed } from 'lucide-react';
...
const DashboardHome: React.FC<DashboardHomeProps> = ({ trips, onSelectTrip, onAddTrip, displayName }) => {
  ...
  // Upcoming trips (future or ongoing, sorted by start_date)
  ...
  // Recent activity: last 5 events across all trips
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto" dir="rtl">
      ...
      <div className="mb-8 text-center sm:text-right">
        ...
        <p className="text-muted-foreground mt-1">
          {upcomingTrips.length > 0
            ? `יש לך ${upcomingTrips.length} הרפתקאות בקרוב. מוכן לצאת?`
            : 'אין טיולים מתוכננים. הגיע הזמן לתכנן!'}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6">
        ...
        <div className="space-y-8">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {upcomingTrips.map(trip => {
                  ...
                  const collaboratorCount = 0; // Could be enhanced later
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center justify-between mt-3">
                          <button className="text-primary text-sm font-medium flex items-center gap-1 hover:underline">
                            פרטי הטיול <ArrowLeft className="h-3.5 w-3.5" />
                          </button>
                        </div>
                      </div>
                    </div>
          ...
          <section>
            <h2 className="text-xl font-bold font-display mb-4">ניתוח הוצאות ופעילות</h2>
            <div className="bg-card border border-border rounded-2xl p-6">
              <div className="text-center mb-4">
                <p className="text-sm text-muted-foreground">סה"כ תקציב מנוצל</p>
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                ...
                <div className="bg-secondary/50 rounded-xl p-3">
                  ...
                  <p className="text-xs text-muted-foreground">פעילויות</p>
                </div>
                <div className="bg-secondary/50 rounded-xl p-3">
                  <p className="text-2xl font-bold">{new Set(trips.map(t => t.destination).filter(Boolean)).size}</p>
                  <p className="text-xs text-muted-foreground">יעדים</p>
                </div>
              </div>
            </div>
          </section>
          ...
          {upcomingTrips.length > 0 && (() => {
            ...
            return (
              <div className="bg-primary rounded-2xl p-5 text-primary-foreground">
                <p className="text-sm opacity-80 mb-1">הטיול הקרוב</p>
                ...
                <div className="mt-4 flex items-end gap-2">
                  ...
                  <span className="text-sm opacity-80 pb-1">ימים</span>
                </div>
          ...
          <div className="bg-card border border-border rounded-2xl overflow-hidden">
            <div className="h-40 bg-secondary flex items-center justify-center relative">
              ...
              <div className="relative text-center">
                <p className="font-bold text-foreground text-sm">לאן תרצה לטוס?</p>
                <p className="text-xs text-muted-foreground mt-1">גלה יעדים חדשים בהתאמה אישית</p>
              </div>
            </div>
          </div>
        </div>
